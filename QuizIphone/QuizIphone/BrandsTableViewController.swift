...
class BrandsTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cellTable", for: indexPath) as! BrandTableViewCell

        // Configure the cell...
        //deklarasi dataTask sebagai Index dari tasks
        let dataTask = tasks[indexPath.row]
        //mengambildata dengan attribute name_Task
        if let myDataTask = dataTask.name_phone {
            //menampilkan data ke label
            cell.labelName.text = myDataTask
        }
        if let myDataTask1 = dataTask.seri_brand {
            cell.labelSeries.text = myDataTask1
        }
        if let myDataTask2 = dataTask.name_brand {
            cell.labelBrand.text = myDataTask2
        }
